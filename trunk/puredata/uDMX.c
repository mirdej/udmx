...
void *uDMX_class;					// global pointer to the object class - so $ can reference the object 
	...
	if (!(x->dev_handle)) find_device(x);
		...
		for(i=0; i<ac; ++i,av++) {
			if (av->a_type==A_FLOAT)
				if (av->a_w.w_float < 0) 	buf[i] = 0;
				else if (av->a_w.w_float > 255) 	buf[i] = 255;
				else buf[i] = av->a_w.w_float;   // MIN(MAX(av->a_w.w_float, 0), 255) doesn't seemto work on mac os x ????;
			else
				buf[i] = 0;
		}
		nBytes = usb_control_msg(x->dev_handle, USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_ENDPOINT_OUT,
