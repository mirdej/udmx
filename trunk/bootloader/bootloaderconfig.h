...
static inline void  bootLoaderInit(void)
{
  ...
  // initialize hardware, same as uDMX
    PORTB = 0x00;       
    DDRB  = 0xFF;          
	PORTC = 0x10;		 // Light yellow led to show were in bootloader
	DDRC  = 0xFF;          
    PORTD = 0x20;       
    DDRD  = 0xD3;
...
//    PORTD |= (1 << 7);			     /* activate usb */
//	softjumper = (MCUCSR & (1 << EXTRF));
    // see if we have softjumper flag in EEPROM		
	//	while(EECR & (1 << EEWE));
	//	EEARL = UBOOT_SOFTJUMPER_ADDRESS;
	//	EECR |= 1 << EERE;
	//	softjumper = EEDR;
//		if (EEDR == UBOOT_SOFTJUMPER) leaveBootloader();
